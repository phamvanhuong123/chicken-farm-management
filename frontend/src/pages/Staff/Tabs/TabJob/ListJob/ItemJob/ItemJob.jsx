import { ChevronsUpDown, MoreHorizontal, Clock, CheckCircle2, Circle } from "lucide-react";
...
const TaskCard = ({ title, description, assignee, status, hasMenu }) => {
  // const statusColor = 
  ...
  return (
    <div className={`group relative bg-white p-3 rounded-lg border border-slate-200 border-l-4 ${statusColor[status]} shadow-sm hover:shadow-md transition-all cursor-pointer`}>
      ...
      <div className="flex items-center justify-between mt-auto">
        ...
        <div className="flex items-center gap-2">
           <div className="h-6 w-6 rounded-full bg-emerald-100 flex items-center justify-center text-[10px] font-bold text-emerald-700">
              {assignee ? assignee.charAt(0) : "U"}
           </div>
           <span className="text-xs font-medium text-slate-600">{assignee || "Chưa giao"}</span>
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex items-center justify-between">
        ...
        <CollapsibleTrigger asChild>
          <Button variant="ghost" size="sm" className="hover:bg-slate-200">
            <span className="text-xs mr-2 text-slate-500">{isOpen ? "Thu gọn" : "Mở rộng"}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between mb-1 px-1">
                ...
                <span className="bg-slate-100 text-slate-500 text-xs px-2 py-0.5 rounded-md">4</span>
            </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between mb-1 px-1">
                ...
                <span className="bg-slate-100 text-slate-500 text-xs px-2 py-0.5 rounded-md">0</span>
            </div>
             <div className="bg-slate-100/50 p-2 rounded-xl  border-slate-100 min-h-[200px] flex flex-col gap-3 items-center justify-center text-slate-400 text-xs border-dashed border-2">
                Trống
            </div>
          </div>
